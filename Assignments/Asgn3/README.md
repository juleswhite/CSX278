...
+ Add the `war` file generated by building your _JHipster_ project in [production mode](https://jhipster.github.io/production/) to the working dir using the `ADD` command. Your file is in `build/libs/`. The syntax od the `ADD` command is `ADD <source_folder> <destination_folder>`. 
+ Add a line `EXPOSE 8080` to expose port 8080 by default.
+ As the last line of your Dockerfile use `CMD` to run the war file generated by jhipster. To run this you'll have to call `java` with the `jar` flag. You should also add `--spring.profiles.active=dev,swagger` at the end of the command to ensure your app is in dev mode, otherwise the web service will fail because of not having a database linked to it. 
+ Build your image with `docker build -t <your_docker_hub_user>/jhipster-project .`
+ Run `docker run -P -it --rm <your_docker_hub_user>/jhipster-project` to run your app. If you pass the flag `-i` it will run in interactive mode (with `-t` it binds the STDIN), if you pass `-d` it runs in detached (daemon) mode. 
+ Kill the running container with `CTRL + C`.
